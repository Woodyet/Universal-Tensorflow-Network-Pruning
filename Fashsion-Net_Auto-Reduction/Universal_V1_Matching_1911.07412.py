...
def initModel_With_layers(layer_numbers):
    ...
    x1 = Dense(L1,name="Dense1_prune",kernel_regularizer=l2(0.001))(Flt)
    x2 = Dense(L2,name="Dense2_prune",kernel_regularizer=l2(0.001))(x1)
...
def compile_np_model(model):
    lr_schedule = keras.optimizers.schedules.ExponentialDecay(initial_learning_rate=0.01,decay_steps=decay_steps,decay_rate=0.1)
...
def train_eval_np_model(model,x_train,y_train,epochs,base_dir):
    ...
    model.fit(x_train,y_train,batch_size=batch_size,epochs=epochs,callbacks=[model_checkpoint_callback])
...
def retrain(weights,model_layers,x_train,y_train,retrain_epochs,prune_logdir,prune_loop,x_val,y_val,conn):
    ...
    model.fit(x_train,y_train, batch_size=batch_size,epochs=retrain_epochs,callbacks=[model_checkpoint_callback])
...
if __name__ == "__main__":
    ...
    base_dir =     'C:\\temp\\mnist\\LE-NET-300-100(mnist-Global-Noise)\\'
    prune_logdir = 'C:\\temp\\mnist\\LE-NET-300-100(mnist-Global-Noise)\\pruned\\'
    finished_dir = 'C:\\temp\\mnist\\LE-NET-300-100(mnist-Global-Noise)\\finished\\'
